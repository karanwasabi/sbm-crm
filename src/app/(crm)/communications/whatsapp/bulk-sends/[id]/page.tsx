...
import { getBulkLeadWhatsAppSendJob } from '@/utils/api';

export default async function WhatsAppBulkSendJobPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  let job;
  try {
    job = await getBulkLeadWhatsAppSendJob(id);
  } catch {
    notFound();
  }
