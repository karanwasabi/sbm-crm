...
function statusTone(status: string): 'brand' | 'neutral' | 'success' {
...
export default async function FeedbackPage() {
  let surveys: Awaited<ReturnType<typeof listSurveys>> = [];
  let loadError: string | null = null;
  try {
    surveys = await listSurveys();
  } catch {
    loadError = 'Surveys are unavailable right now. Please try again.';
  }
    ...
    <CrmPageLayout className="gap-4">
      ...
      {!loadError && surveys.length === 0 ? (
        <Card>
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">No surveys yet.</p>
          </div>
        </Card>
      ) : null}
      ...
      {!loadError && surveys.length > 0 ? (
        <div className="flex flex-col gap-3">
          {surveys.map((survey) => (
            <Card key={survey.id} padding="sm" className="overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex min-w-0 flex-1 items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-extrabold text-slate-800">{survey.title}</p>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <Pill tone={statusTone(survey.status)}>{survey.status}</Pill>
                      <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">
                        {formatSurveyDate(survey.starts_on)} – {formatSurveyDate(survey.ends_on)}
                      </span>
                    </div>
