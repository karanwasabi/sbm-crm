...
function PurgeAuditDetailDialog({
  ...
  const billing = detail.billing_snapshot;
  const razorpay = detail.razorpay_snapshot;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-2xl">
        ...
        <div className="max-h-[min(75vh,640px)] space-y-4 overflow-y-auto px-6 py-5">
          ...
          {billing.checkouts && billing.checkouts.length > 0 ? (
            <DetailSection title="Checkout sessions">
              <div className="space-y-3">
                {billing.checkouts.map((checkout) => (
                  <div key={checkout.checkout_session_id} className="rounded-lg border border-slate-100 bg-white p-3">
                    ...
                    {checkout.razorpay_subscription_id ? (
                      <p className="mt-1 text-xs break-all text-slate-500">Sub {checkout.razorpay_subscription_id}</p>
                    ) : null}
          ...
          {billing.recurring_charges && billing.recurring_charges.length > 0 ? (
            <DetailSection title="Recurring charges">
              <div className="space-y-2">
                {billing.recurring_charges.map((charge, index) => (
                  <div
          ...
          {razorpay.subscriptions && razorpay.subscriptions.length > 0 ? (
            <DetailSection title="Razorpay">
              <div className="space-y-2">
                {razorpay.subscriptions.map((sub) => (
                  <div key={sub.subscription_id} className="rounded-lg border border-slate-100 bg-white p-3 text-xs">
                    ...
                    {sub.tokens_revoked && sub.tokens_revoked.length > 0 ? (
                      <p className="mt-1 break-all text-slate-500">Tokens revoked: {sub.tokens_revoked.join(', ')}</p>
                    ) : null}
                  </div>
                ))}
              </div>
...
export function PurgeAuditPanel({ initialItems, total }: PurgeAuditPanelProps) {
  ...
  return (
    <>
      ...
      <PurgeAuditDetailDialog detail={detail} open={detailOpen} onOpenChange={setDetailOpen} />
    </>
  );
}
